...
useEffect(()=>{
  ...
  const confirmTopUp = () => {
  if (user_id && topUpAmount) {
    // Update saldo
    axios.post(`http://127.0.0.1:8000/api/users/${user_id}/update-balance`, { amount: parseFloat(topUpAmount) })
      .then((response) => {
          setBalance(response.data.data.saldo_dana || 0)
          toast({
            title: "Top-up berhasil",
            description: "Saldo berhasil diperbarui dan entri top-up telah dicatat.",
            status: "success",
            duration: 5000,
            isClosable: true,
          });
      ...
      .catch((error) => {
        toast({
          title: "Top-up gagal",
          description: error.response?.data?.message || "Terjadi kesalahan saat memperbarui saldo.",
          status: "error",
          duration: 5000,
          isClosable: true,
        });
  ...
  const TopUpHistoryPopup = ({ isOpen, onClose, topUpHistory }) => (
        ...
        <VStack>
          <Box w={"90%"}>
            ...
            <HStack mt={5}>
              <Spacer />
              <FormControl>
                <Text fontSize="lg" >{t("profile.balance")}: Rp.{balance}</Text>
                <HStack mb={6}>
        ...
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              ...
              <Text mt={4}>{t("profile.confirmTopUpAmount")}: Rp. {topUpAmount}</Text>
            </AlertDialogBody>
            <AlertDialogFooter>
